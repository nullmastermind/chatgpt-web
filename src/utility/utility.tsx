...
export function doc2ChatContent(doc: Doc, score: number) {
  ...
  if (
    doc.pageContent.startsWith('DOCUMENT NAME:') ||
    doc.pageContent.startsWith('REFERENCE CODE:')
  ) {
    offsetLine.value = 2;
  }
...
export const preprocessMessageContent = (content: string): string => {
  ...
  if (contentArr[contentArr.length - 1].endsWith('```')) {
    contentArr[contentArr.length - 1] = contentArr[contentArr.length - 1].replace(
      /(.*?)```$/,
      '$1\n```',
    );
  }
